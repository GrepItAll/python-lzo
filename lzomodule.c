...
compress(PyObject *dummy, PyObject *args)
{
    ...
    if (level == 1)
        wrkmem = (lzo_voidp) PyMem_Malloc(LZO1C_1_MEM_COMPRESS);
    else
        wrkmem = (lzo_voidp) PyMem_Malloc(LZO1C_999_MEM_COMPRESS);
    if (wrkmem == NULL)
    {
    ...
    if (level == 1)
    {
        if (header)
            out[0] = 0xf0;
        err = lzo1c_1_compress(in, in_len, outc, &new_len, wrkmem);
    }
    else
    {
        if (header)
            out[0] = 0xf1;
        err = lzo1c_999_compress(in, in_len, outc, &new_len, wrkmem);
    }
...
decompress(PyObject *dummy, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
    new_len = out_len;
    err = lzo1c_decompress_safe(in, in_len, out, &new_len, NULL);
    Py_END_ALLOW_THREADS
...
optimize(PyObject *dummy, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
    new_len = out_len;
    err = lzo1c_optimize( header ? in+5 : in, in_len, out, &new_len, NULL);
    Py_END_ALLOW_THREADS
